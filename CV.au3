...
#pragma compile(Icon, 'Resources\100.cur', 100)
#pragma compile(Icon, 'Resources\201.ico', 201)
#pragma compile(Icon, 'Resources\202.ico', 202)
...
#NoTrayIcon
; ==============================================================================
; 快捷鍵與全域狀態初始化 (請置於 CV.au3 各 include 下方、GUI 建立前)
Global $copy_title_class,$copy_CTRL_INSTANCE,$copy_CTRL_ID,$checkboxquotesforcopy
Global $copy_CTRL_SD,$copy_CTRL_CK,$sCtr_Ck,$sCtr_SD
Global $bFrozen = False
Global $BTool=0
Global $myLabel=""
...
Global $AutoSpy = 0 ;2000 ; SPY about every 2000 milliseconds automatically, 0 is turn of use only ctrl+w

Global $oldUIElement ; To keep track of latest referenced element
Global $frmSimpleSpy, $edtCtrlInfo, $lblCapture, $lblEscape, $msg, $x, $y, $oUIElement, $oTW, $objParent, $oldElement, $text1, $t
Local $i ; Just a simple counter to measure time expired in main loop
Global $UIA_CodeArray

Global $UIA_oTW ; Generic treewalker which is allways available
Global $UIA_oTRUECondition ; TRUE condition easy to be available for treewalking
Global $sIconRelease = @ScriptDir & '\Resources\201.ico' ; 放開時的圖示
Global $sIconPush    = @ScriptDir & '\Resources\202.ico' ; 按下時的圖示
;-------------------------------------------------------------------------------------------
...
While 1
	...
	Switch $Msg
		...
		Case $GUI_EVENT_CLOSE
			Exit
		case $copy_CTRL_INSTANCE  ;----------------------------------------cvpatch
			if GUICtrlRead($checkboxquotesforcopy)=$GUI_UNCHECKED then

			ClipPut('"'&GUICtrlRead($Input[18])&'"')
			Else
			ClipPut("'"&GUICtrlRead($Input[18])&"'")
		EndIf
		...
		case $copy_CTRL_ID
			if GUICtrlRead($checkboxquotesforcopy)=$GUI_UNCHECKED then
			ClipPut('"'&GUICtrlRead($Input[20])&'"')
			Else
				ClipPut("'"&GUICtrlRead($Input[20])&"'")
			EndIf
		case $copy_title_class


			$cliptmp = ""
			$titletmp = GUICtrlRead($Input[3])
		...
		;---------- mod:Shen@20260802
		Case $checkboxquotesforcopy
			$BTool=($BTool ? 0 : 1)
			$Data=($BTool ? GUICtrlSetData($myLabel, 'BTool : 1') : GUICtrlSetData($myLabel, 'BTool : 0') )
		case $copy_CTRL_CK
			If GUICtrlRead($Input[3])="" Then ContinueLoop
			CTRL_CK()
			ClipPut($sCtr_Ck)
		case $copy_CTRL_SD
			If GUICtrlRead($Input[3])="" Then ContinueLoop
		...
		Case $Icon[0]
			...
			While _IsPressed($PrimaryMouseButton) Or $BTool
...
Func _GUICreate()
	...
	; Main Window
	$hForm = GUICreate($GUI_NAME & ChrW(160), $_Width+30, $_Height + _WinAPI_GetSystemMetrics($SM_CYMENU), $_XPos, $_YPos, BitOR($GUI_SS_DEFAULT_GUI, $WS_MAXIMIZEBOX, $WS_SIZEBOX), $WS_EX_TOPMOST * $_Top)
	...
	; Info Group
	If $_Icon Then
		$Tab = GUICtrlCreateTab(22, 136, 399, 273)
		_GUICtrlTab_SetMinTabWidth(-1, 75)
	Else
		$Tab = GUICtrlCreateTab(22, 136, 399, 273, BitOR($GUI_SS_DEFAULT_TAB, $TCS_FIXEDWIDTH))
	...
	GUICtrlCreateGroup('Info', 10, 115, 431, 305)

 	$copy_title_class=GUICtrlCreateButton("WIN TITLE+CLASS",15,110,100,20)
 	$copy_CTRL_INSTANCE=GUICtrlCreateButton("^INSTANCE",120,110,69,20)
 	$copy_CTRL_ID=GUICtrlCreateButton("^ID",190,110,45,20)
 	$checkboxquotesforcopy=GUICtrlCreateCheckbox("",240,113,15,15)
 	$copy_CTRL_CK=GUICtrlCreateButton("^Click",261,110,60,20)
 	$copy_CTRL_SD=GUICtrlCreateButton("^Send",330,110,60,20)
	; Controls Group
	$Group = GUICtrlCreateGroup('Controls', 10, 424, 431, 219 + $Height)
...
; ==============================================================================
; 將以下這兩個函式完整覆蓋至 CV.au3 檔案的最下方 (All Func 置最下方)
; ==============================================================================

; 🛠️ 1. 快顯選單擷取與凍結核心 (Ctrl+Alt+I)
Func CapturePopupControl()
    $bFrozen = Not $bFrozen
...
; 🔒 2. 內部輔助函式：當處於解凍自由狀態時，強行呼叫原廠雙核心即時更新全畫面欄位
Func BackgroundMouseMoveRefresh()
    ; 安全防護：如果處於凍結狀態，或變數未建立，直接 Return 不干擾畫面
    If $bFrozen Or (Not IsDeclared("Input")) Then Return
    ...
    DllStructSetData($tPoint, "X", $aMousePos[0])
    DllStructSetData($tPoint, "Y", $aMousePos[1])

    Local $hWindow = _WinAPI_WindowFromPoint($tPoint)
    If Not $hWindow Then Return

    _WinAPI_ScreenToClient($hWindow, $tPoint)
    Local $aResult = DllCall("user32.dll", "hwnd", "ChildWindowFromPointEx", _
            "hwnd", $hWindow, "long", DllStructGetData($tPoint, "X"), "long", DllStructGetData($tPoint, "Y"), "uint", 0x0001)

    Local $hControl = (Not @error And $aResult) ? $aResult : $hWindow

    ; 2. 即時寫入 Color Picker 分頁中的 X, Y 座標與顏色
    GUICtrlSetData($Input[0], $aMousePos[0])
    GUICtrlSetData($Input[1], $aMousePos[1])
    Local $hDC = _WinAPI_GetDC(0)
    Local $iColorInt = _WinAPI_GetPixel($hDC, $aMousePos[0], $aMousePos[1])
    _WinAPI_ReleaseDC(0, $hDC)
    GUICtrlSetData($Input[2], "0x" & Hex($iColorInt, 6))

    ; 3. 💡【終極同步開關】直接呼叫原廠雙核心更新，讓 Class、Instance 隨滑鼠移動全自動跳動刷新
    _SetWindowInfo($hWindow)
    _SetControlInfo($hControl)
EndFunc

; 🖱️ 2. 輸出 ControlClick 語法 (Ctrl+Alt+K)
Func ExportControlClick()
    Local $sControlID = _GetTargetControlID()
    If $sControlID = "" Then Return

    ; 嚴格讀取對應陣列：$Input[3] = Title, $Input[0] = X, $Input[1] = Y
	Local $sTitle = StringRegExpReplace(GUICtrlRead($Input[3]),"\s.*","")
    Local $sOutput = 'ControlClick("' & $sTitle & '", "", "' & $sControlID & '", "left", 1, ' & GUICtrlRead($Input[0]) & ', ' & GUICtrlRead($Input[1]) & ')'
    _WriteToCtrClkFile($sOutput, "ControlClick 語法")
EndFunc

; 📋 3. 輸出 獲取數值/文字 語法 (Ctrl+Alt+V)
Func ExportControlValue()
    Local $sControlID = _GetTargetControlID()
    If $sControlID = "" Then Return

    ; 讀取 $Input[3] = Title，並生成標準格式
	Local $sTitle = StringRegExpReplace(GUICtrlRead($Input[3]),"\s.*","")
    Local $sOutput = '$sVal = ControlGetText("' & $sTitle & '", "", "' & $sControlID & '")'
    _WriteToCtrClkFile($sOutput, "ControlGetText 語法")
EndFunc

;  ⌨️  4. 彈出 InputBox 輸出 Click + Send 組合 (Ctrl+Alt+S)
Func ExportClickAndSend()
    Local $sControlID = _GetTargetControlID()
    If $sControlID = "" Then Return

    ; 彈出輸入方塊讓使用者填寫要發送的內文
    Local $sTextToSend = InputBox("Control Viewer 輸出模組", "請輸入您希望透過 ControlSend 發送的文字：", "", "", 280, 130)
    If @error Then Return ; 使用者點擊「取消」或關閉則不動作

    Local $sTitle = StringRegExpReplace(GUICtrlRead($Input[3]),"\s.*","")
    Local $sX = GUICtrlRead($Input[0])
    Local $sY = GUICtrlRead($Input[1])

    ; 組合成兩行自動化指令：先點擊、後發送
    Local $sOutput = 'ControlClick("' & $sTitle & '", "", "' & $sControlID & '", "left", 1, ' & $sX & ', ' & $sY & ')' & @CRLF & _
                     'ControlSend("' & $sTitle & '", "", "' & $sControlID & '", "' & $sTextToSend & '")'

    _WriteToCtrClkFile($sOutput, "Click + Send 複合語法")
EndFunc

; 🔒  內部私有輔助函式A：自動計算最精準的 ControlID 識別碼格式
Func _GetTargetControlID()
    If Not IsDeclared("Input") Then
        Beep(300, 100)
        Return ""
    EndIf

    Local $sClass = GUICtrlRead($Input[16])   ; 讀取 Control 分頁的 Class
    Local $sInstance = GUICtrlRead($Input[18]); 讀取 Control 分頁的 Instance

    ; 如果 Instance 是純數字，自動包裝成標準 AutoIt 語法，否則直接回傳
    If StringIsDigit($sInstance) Then
        Return "[CLASS:" & $sClass & "; INSTANCE:" & $sInstance & "]"
    EndIf
    Return $sInstance
EndFunc

; 💾  內部私有輔助函式B：通用的寫入檔案與提示音效管理
Func _WriteToCtrClkFile($sText, $sMsgType)
    Local $sFilePath = @ScriptDir & "\CtrClk.txt"
    Local $hFile = FileOpen($sFilePath, 1) ; 1 = 開啟檔案至追加寫入模式
    If $hFile <> -1 Then
        FileWriteLine($hFile, $sText)
        FileClose($hFile)

        Beep(1200, 150) ; 成功寫入發出清脆的叮咚高音
        Local $aM = MouseGetPos()
        ToolTip("💾  已將 " & $sMsgType & " 寫入 CtrClk.txt！", $aM[0] + 15, $aM[1] + 15)
        AdlibRegister("_ClearExportToolTip", 1500) ; 1.5 秒後自動收回提示框
    Else
        ToolTip("❌ 寫入失敗，請確認 CtrClk.txt 是否被其他軟體佔用！", 10, 10)
        Beep(300, 200)
    EndIf
EndFunc

; 專用清除提示的 Adlib 輔助小功能
Func _ClearExportToolTip()
    ToolTip("")
    AdlibUnRegister("_ClearExportToolTip")
EndFunc
...
Func CTRL_CK()
	$sCtr_Ck=""
	Local $titletmp = StringRegExpReplace(GUICtrlRead($Input[3]),"\s.*","")
...
Func CTRL_SD()
	...
	Local $Data = GUICtrlRead($Input[30])
	$Data=StringLeft(StringRegExpReplace($Data,"\s.*",""),10)
	Local $cliptmp='ControlSend("' & $titletmp & '" , "' & $Data & '" , "'
	$Data = ""
	If GUICtrlRead($Input[20]) Then
		$Data = GUICtrlRead($Input[20])
	ElseIf GUICtrlRead($Input[18]) Then
		$Data = GUICtrlRead($Input[18])
	EndIf
	$cliptmp &=$Data &'" , "$Value" )'
	$sCtr_SD = $cliptmp
	Return $cliptmp
...
Func GetElementInfo()
	...
	If IsObj($oUIElement) Then
		...
		If $BTool=0 Then GUICtrlSetData($edtCtrlInfo,'')	;---mod:Shen@20260804
		...
		If $nativeWindow <> 0 Then
			$codeText1 &= "_UIA_setVar(""" & $controlIDString & ".mainwindow"",""title:=" & $title & ";classname:=" & $class & """)" & @CRLF
			$codeText1 &= "_UIA_action(""" & $controlIDString & ".mainwindow"",""setfocus"")" & @CRLF
		Else
		...
		$text1 &= "*** Parent Information top down ***" & @CRLF
		Local $pText1 = ""
		Local $pCodeText2 = ""
		...
		$text1 &= "*** Standard code ***" & @CRLF
		$text1 &= "#include ""UIAWrappers.au3""" & @CRLF
		$text1 &= "AutoItSetOption(""MustDeclareVars"", 1)" & @CRLF & @CRLF
		...
		EndIf
		;---------------------------
		$text1 &= "*** Detailed properties of the highlighted element ***"
		$text1 &= @CRLF & _UIA_getAllPropertyValues($oUIElement)
